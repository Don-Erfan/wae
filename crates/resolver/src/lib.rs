...
mod resolution_kind;
mod tsconfig;
pub use builtin::{BuiltinCatalog, BuiltinResolver};
pub use conditions::{
...
pub use tsconfig::{IndexedAliasResolver, TsConfigIndex, TsConfigLoader, TsConfigPaths};
...
mod tests {
    ...
    fn node_builtins_are_distinct_from_external_packages() {
        ...
        assert_eq!(
            resolver.resolve(&request(&importer, "node:sqlite")),
            Resolution::Builtin("node:sqlite".into())
        );
        assert_eq!(
            resolver.resolve(&request(&importer, "node:test/reporters")),
            Resolution::Builtin("node:test/reporters".into())
        );
        assert_eq!(
            resolver.resolve(&request(&importer, "test")),
            Resolution::External("test".into())
        );
        assert_eq!(
